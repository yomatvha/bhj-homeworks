...
adds.forEach((addButton) => {
  addButton.addEventListener("click", () => {
    ...
    for (let i = 0; i < cartProducts.length; i++) {
      if (cartProducts[i].getAttribute('data-id') === prodId) {
        let cnt = cartProducts[i].querySelector('.cart__product-count').textContent;
        cartProducts[i].querySelector('.cart__product-count').textContent = (Number(cnt) + Number(amnt)).toString();
        return;
      }
    }
